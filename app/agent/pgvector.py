...
class PostgresRAG:
    ...
    async def query(self, q: str, top_k: int = 3) -> list[Document]:
        try:
            ...
            ranked_docs = self.rerank_results(key_tokens=keywords)
            doc_texts = [doc.page_content for doc in ranked_docs]
            # print(ranked_docs)
            # print(doc_texts)
            cohere_ranked_docs = await self.cohere_client.rerank(query=q, documents=doc_texts, top_n=top_k, model="rerank-v3.5")
            indices = []
            for response in cohere_ranked_docs.results:
                indices.append(response.index)
            cohere_ranked_docs = [ranked_docs[i] for i in range(len(ranked_docs)) if i in indices]
            return cohere_ranked_docs
...
async def main() -> None:
    ...
    results = await db.query("What is Jason's linkedin?")
    print(results)
